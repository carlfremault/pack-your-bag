...
async function bootstrap() {
  ...
  await assertDlqQueue(rabbitMQUrl, RMQ_QUEUES.USER_CLEANUP_USER_DATA);
  app.connectMicroservice(getRmqConsumerOptions(rabbitMQUrl, RMQ_QUEUES.USER_CLEANUP_USER_DATA));

  // Graceful shutdown
  app.enableShutdownHooks();

  await app.startAllMicroservices();
  await app.listen(process.env.UDS_PORT ?? 8003);
}
