...
export class UserEmailListener {
  ...
  async handleAccountDeletionRequested(event: AccountDeletionRequestedEvent): Promise<void> {
    ...
    const resetLink = `${this.frontendUrl}/cancel-deletion?token=${encodeURIComponent(cancellationToken)}`;

    await this.emailService.sendEmailWithRetry(
      {
        to: email,
        subject: 'Account Deletion Request',
        text: `Your account deletion has been scheduled for ${cancellationDate}. If this was a mistake you can cancel the deletion here: ${resetLink}. After ${cancellationDate}, your account and all data will be permanently deleted.`,
        html: `
          <p>Your account deletion has been scheduled for ${cancellationDate}.</p>
          <p><strong>If this was a mistake you can <a href="${resetLink}">cancel the deletion here</a>.</strong></p>
